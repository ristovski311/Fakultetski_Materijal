...
void CGLKView::OnKeyDown(UINT nChar, UINT nRepCnt, UINT nFlags)
{
	...
	if (nChar == 'G')
		m_glRenderer.greenLightOn = !m_glRenderer.greenLightOn;
...
BOOL CGLKView::OnMouseWheel(UINT nFlags, short zDelta, CPoint pt)
{
	...
	m_glRenderer.cameraRadius -= zDelta * zoomSensitivity;

	if (m_glRenderer.cameraRadius > 5000)
		m_glRenderer.cameraRadius = 5000;

	if (m_glRenderer.cameraRadius < 1)
		m_glRenderer.cameraRadius = 1;
...
void CGLKView::OnMouseMove(UINT nFlags, CPoint point)
{
	if (m_glRenderer.isMouseDown)
	{
		...
		m_glRenderer.cameraAngleX += dy * rotationSensitivity;
		m_glRenderer.cameraAngleY -= dx * rotationSensitivity;

		if (m_glRenderer.cameraAngleX > 90 * TORAD)
			m_glRenderer.cameraAngleX = 90 * TORAD;
		if (m_glRenderer.cameraAngleX < -90 * TORAD)
			m_glRenderer.cameraAngleX = -90 * TORAD;
